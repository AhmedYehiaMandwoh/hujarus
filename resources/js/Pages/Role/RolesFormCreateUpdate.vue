    ...
    <form @submit.prevent="submit()">
        ...
        <main class="grid grid-cols-3 gap-3">
            <template v-for="(items, index) of formData.abilities">
                <ElCardWithTitle class="max-h-[350px] overflow-x-auto" :title="$t('enums.ModuleNameEnum.' + index)">
                    <template #actions>
                        <template v-if="(items.map((x) => x.key)).every((elem) => el_form.abilities.includes(elem))">
                            <secondary-button
                                @click="toggleSelect(index, items, 0);"
                                class="!p-1"
                        ...
                        <template v-else>
                            <secondary-button
                                @click="toggleSelect(index, items, 1);"
                                class="!p-1"
                    ...
                    <aside class="grid md:grid-cols-2 gap-2">
                        <template v-for="ability of items" :key="ability.key">
                            <template
                                v-if="
                                    !$page.props.ignored_department_abilities ||
                                    !$page.props.ignored_department_abilities.includes(ability.key)
                                "
                            >
        ...
        <span v-if="el_form.errors.abilities" class="text-red-500">
            {{ el_form.errors.abilities[0] }}
        </span>
