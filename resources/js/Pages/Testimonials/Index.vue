...
import ElRouteTestimonials from "@/Components/ElRoutes/ElRouteTestimonials.vue";
...
<template>
    <ElPanel :is-transparent="false">
        <template #actions>
            <primary-button :ability="Ability.M_TESTIMONIALS_CREATE" :href="route('testimonials.create')" :text="$t('base.add_new')" />
            <ExportExcelButton :ability="Ability.M_TESTIMONIALS_INDEX_EXPORT"/>

        </template>
        <ElDataTable :src="props.data">
            <Column :header="$t('base.title')" :alignFrozen="getAlignFrozen()" frozen>
                <template #body="row">
                    <ElRouteTestimonials :model="row.data" />
                </template>
            </Column>
            ...
            <Column :header="$t('base.actions')">
                <template #body="row">
                    <ActionMenu>
                        <ActionMenuEdit :ability="Ability.M_TESTIMONIALS_UPDATE" :href="route('testimonials.profile.edit', row.data.id)" />
                        <ActionMenuDeleteAction :ability="Ability.M_TESTIMONIALS_DELETE" :dialog-message="row.data.name"
                            :href="route('testimonials.delete', row.data.id)" />
                    </ActionMenu>
                </template>
            </Column>
        </ElDataTable>
